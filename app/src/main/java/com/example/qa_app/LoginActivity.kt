...
class LoginActivity : AppCompatActivity() {
    ...
    private var mIsCreateAccount = false //名前を保存できる様にする /falseにする理由は？

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_login)

        mDataBaseReference = FirebaseDatabase.getInstance().reference //データベースにFireBase作成

        mAuth = FirebaseAuth.getInstance() // firebaseのオブジェクトを作成
        ...
        mLoginListener = OnCompleteListener { task ->
            if (task.isSuccessful) {
                ...
                val userRef = mDataBaseReference.child(UsersPATH).child(user!!.uid) //意味を理解できていない
